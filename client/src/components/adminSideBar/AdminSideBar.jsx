    ...
    <>
        <div className={`xl:h-[100vh] overflow-y-scroll text-artistfont fixed xl:static md:w-[40%] ls:w-[30%] xl:w-auto w-[80%] h-full top-0 bg-secondary-900 p-4 flex flex-col justify-between z-40 ${showMenu ? "left-0" : "-left-full"} transition-all`}>
          <div>
            ...
            <ul>
            <li>
                  <Link href='/admin-dashboard/registeredArtists' className='w-full flex items-center rounded-md border-b-[1px] border-admin/20 font-newrocker justify-between gap-4 px-4 py-5  hover:bg-secondary-100 transition-colors'>
              ...
              <li>
                  <Link href='/admin-dashboard/tattooStyles' className='flex items-center gap-4 px-4 py-5 rounded-md font-newrocker border-b-[1px] border-admin/20 hover:bg-secondary-100 transition-colors'>
        ...
        <button onClick={()=>setShowMenu(!showMenu)} className=' xl:hidden fixed bottom-4 right-4 bg-admin text-black p-4 rounded-full z-40'>
            {
              showMenu
              ?<RiCloseFill/>
              :<RiMenuFill/>
