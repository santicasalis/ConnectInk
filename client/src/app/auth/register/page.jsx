  ...
  return (
    <div className="bg-secondary-900/90 flex opacity-90 absolute h-[80vh] rounded-3xl w-full xl:w-1/2 lg:w-1/3 md:w-1/2">
        <div className="w-[35%] border-transparent border-r-[1px] border-r-white/10 flex flex-col items-center justify-center text-center px-8">
              <h2 className="font-rocksalt text-[40px] text-white/90 mb-2">Registrate </h2> 
              <p className="text-primary/80 mb-4">como:</p> 
              <div className="flex mb-4">
                <button
                  onClick={() => setClient(true)}
                  className={`text-white font-newrocker text-[17px]  py-3 px-4 font-bold rounded-l-2xl ${client ? "bg-primary/90 text-[22px]" : "bg-secondary-100 text-white/80" }`}
                >
                  Cliente
                </button>
                <button
                  onClick={() => setClient(false)}
                  className={`text-white py-3 px-4 font-newrocker text-[17px]  font-bold rounded-r-2xl ${client ? "bg-secondary-100 text-white/80" : "bg-primary/90 text-[22px]" }`}
                >
                  Artista
                </button>
              </div>
              {client ? (
                  <div>
                    <p className="text-white/80">
                      "Como cliente, tendrás la posibilidad de buscar a
                      ...
                      con los tatuadores, seleccionar tatuadores que te gusten, y tener comunicacion directa con el artista luego
