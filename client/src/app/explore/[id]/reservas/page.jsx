...
const bookAppointment = ({ params }) => {
  ...
  const getHours = () => {
    ...
    artist?.appointments?.forEach((appointment) => {
      ...
      let initial =
        objH[appointmentDate.getDay()]?.at(0) ||
        objH[appointmentDate?.toDateString().at(0)];
      let final =
        objH[appointmentDate.getDay()]?.at(-1) ||
        objH[appointmentDate?.toDateString().at(-1)];
      objH[appointmentDate.toDateString()] = createHourArrayWithAppointment(
        initial,
        final,
        appointmentTime,
